...
table(res_N_Flag$padj<0.05 & abs(res_N_Flag$log2FoldChange)>1.5)
#FALSE  TRUE 
#48821   506 
...
table(res_NLS_Flag$padj<0.05 & abs(res_NLS_Flag$log2FoldChange)>1.5)
#FALSE  TRUE 
#49018   550 
...
# FALSE=59047 TRUE=1
table(res_N_NLS$padj<0.05 & abs(res_N_NLS$log2FoldChange)>1.5)
#FALSE 
#59048 
...
rld_pvals_NFlag_name <- rldpvals_NFlag %>%
...
rld_pvals_NLSFlag_name <- rldpvals_NLSFlag %>%
  tibble::rownames_to_column(var = "ISO") %>%
  left_join(iso2gene_good)
```
...
#even though I'm not using this, let's also read in N to NLS
...
N_Sup05_1.5=subset(N_rld_pvals_lfc, N_lfc>1.5 & padj.N_Flag<0.05 & !is.na(N_rld_pvals_lfc$padj.N_Flag))
head(N_Sup05_1.5)
nrow(N_Sup05_1.5)
#451
N_Sdown05_1.5=subset(N_rld_pvals_lfc, N_lfc< -1.5 & padj.N_Flag<0.05 & !is.na(N_rld_pvals_lfc$padj.N_Flag))
head(N_Sdown05_1.5)
nrow(N_Sdown05_1.5)
#55
...
###up and down with lfc set to 1.5
NLS_Sup05_1.5=subset(NLS_rld_pvals_lfc, NLS_lfc>1.5 & padj.NLS_Flag<0.05 & !is.na(NLS_rld_pvals_lfc$padj.NLS_Flag))
head(NLS_Sup05_1.5)
nrow(NLS_Sup05_1.5)
#492
NLS_Sdown05_1.5=subset(NLS_rld_pvals_lfc, NLS_lfc< -1.5 & padj.NLS_Flag<0.05 & !is.na(NLS_rld_pvals_lfc$padj.NLS_Flag))
head(NLS_Sdown05_1.5)
nrow(NLS_Sdown05_1.5)
#58
...
candidates_all05_1.5=list("N"=N_DEGs05_lfc1.5, "NLS"=NLS_DEGs05_lfc1.5)

venn05_1.5=venn.diagram(
  x = candidates_all05_1.5,
  category.names=c("N (506)", "NLS (550)"),
  col=c("#0066CC", "#CC99CC"),
  lwd=c(3,3),
  filename=NULL,
  #main = "DEGs in Comparison to Flag",
...
grid.draw(venn05_1.5)
#saved as venn05_1.5_040624.pdf as landscape with 6x6 dimensions in Figures

#Make a third Venn with the DEGs from the 0.05 padj cutoff + 2 lfc cutoff
candidates_all05_2=list("N"=N_DEGs05_lfc2, "NLS"=NLS_DEGs05_lfc2)

venn05_2=venn.diagram(
  x = candidates_all05_2,
  category.names=c("N (279)", "NLS (296)"),
  col=c("#0066CC", "#CC99CC"),
  lwd=c(3,3),
  filename=NULL,
  #main = "DEGs in Comparison to Flag",
...
grid.draw(venn05_2)
#saved as venn05_2_040624.pdf as landscape with 6x6 dimensions in Figures
```
...
gtf <- read.table("./Data_Files/GRCh38_latest_genomic.gtf", sep="\t")
head(gtf)
