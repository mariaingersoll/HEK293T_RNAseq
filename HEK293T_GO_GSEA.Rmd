...
row.order <- sN_rel_hm$tree_row$order
reorder <- explc_sN_rel[row.order,]
head(reorder)

explc_sNNLS_rel <- rownames_to_column(reorder, var="SYMBOL")
explc_sNNLS_rel <- explc_sNNLS_rel %>%
  left_join(sN_NLS_rel_un) %>%
  right_join(sNLS_new1)

write.csv(explc_sNNLS_rel, file="./Data_Files/explc_sNNLS_rel.csv", quote=F)
```
...
saving file of lfc of all shared genes that were input into enrichr
```{r}
head(shared_both_lfc_new)
nrow(shared_both_lfc_new)
#1192
shared <- rownames_to_column(shared_both_lfc_new, var="REFSEQ") %>%
  left_join(cds_seq2iso_1)
nrow(shared)
#1192
head(shared)
tail(shared)
write.csv(shared, file="./Data_Files/shared_lfc.csv", quote=F)
```
